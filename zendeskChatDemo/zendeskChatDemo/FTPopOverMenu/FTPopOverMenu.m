...
#define FTDefaultMenuIconSize               20.0f
...
+ (FTPopOverMenuConfiguration *)FTConfiguration{
...
- (void)setupWithMenuName:(NSString *)menuName menuImage:(id)menuImage selected:(BOOL)selected {
    ...
    CGRect menuNameRect = CGRectMake(menuNameX, 0, configuration.menuWidth - menuNameX, configuration.menuRowHeight);

    if (!menuImage) {
        CGFloat menuTextX = configuration.menuTextMargin + configuration.contentViewMargin;
        menuNameRect = CGRectMake(configuration.menuTextMargin, 0, configuration.menuWidth - menuTextX*2, configuration.menuRowHeight);
        self.contentView.frame = menuNameRect;
    }else{
    ...
    if (selected) {
        ...
        self.contentView.backgroundColor = configuration.selectedCellBackgroundColor;
    }else{
        self.contentView.backgroundColor = configuration.tintColor;
    }
}
...
- (void)layoutSubviews
{
   [super layoutSubviews];
    self.contentView.layer.cornerRadius = [FTPopOverMenuConfiguration defaultConfiguration].contentViewRadius;
    if ([FTPopOverMenuConfiguration defaultConfiguration].needChangeContentViewFrame) {
        self.contentView.frame = CGRectMake([FTPopOverMenuConfiguration defaultConfiguration].contentViewMargin, 0, [FTPopOverMenuConfiguration defaultConfiguration].menuWidth - [FTPopOverMenuConfiguration defaultConfiguration].contentViewMargin*2, [FTPopOverMenuConfiguration defaultConfiguration].menuRowHeight);
    }
}
...
- (UITableView *)menuTableView {
    if (!_menuTableView) {
        ...
        _menuTableView.backgroundColor = [FTPopOverMenuConfiguration defaultConfiguration].menuBackColor?[FTPopOverMenuConfiguration defaultConfiguration].menuBackColor:FTDefaultBackgroundColor;
        _menuTableView.separatorColor = [FTPopOverMenuConfiguration defaultConfiguration].separatorColor;
        _menuTableView.layer.cornerRadius = [FTPopOverMenuConfiguration defaultConfiguration].menuCornerRadius < 0?FTDefaultMenuCornerRadius:[FTPopOverMenuConfiguration defaultConfiguration].menuCornerRadius;
...
- (void)showWithFrame:(CGRect )frame
    ...
    CGFloat arrowHeight = [FTPopOverMenuConfiguration defaultConfiguration].hideArrow?0:self.menuArrowHeight;
    CGRect menuRect = CGRectMake(0, arrowHeight, self.frame.size.width, self.frame.size.height - arrowHeight);
    if (_arrowDirection == FTPopOverMenuArrowDirectionDown) {
        menuRect = CGRectMake(0, 0, self.frame.size.width, self.frame.size.height - arrowHeight);
    }
    [self.menuTableView setFrame:menuRect];
    [self.menuTableView reloadData];

    if (![FTPopOverMenuConfiguration defaultConfiguration].hideArrow) {
...
- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section {
    return [FTPopOverMenuConfiguration defaultConfiguration].menuHeaderHeight;
}
...
- (CGFloat)tableView:(UITableView *)tableView heightForFooterInSection:(NSInteger)section {
    return [FTPopOverMenuConfiguration defaultConfiguration].menuFooterHeight;
}
...
//    menuCell.contentView.backgroundColor = [FTPopOverMenuConfiguration defaultConfiguration].selectedCellBackgroundColor;
    return menuCell;
}
...
+ (void) showForSender:(UIView *)sender
    ...
    [FTPopOverMenu.new showForSender:sender senderFrame:CGRectNull withMenu:menuArray imageNameArray:nil doneBlock:doneBlock dismissBlock:dismissBlock];
}
...
- (UIView *)backgroundView {
    if (!_backgroundView) {
        ...
        _backgroundView.backgroundColor = [FTPopOverMenuConfiguration defaultConfiguration].backGroundViewColor;
    }
...
- (void)adjustPopOverMenu {
    ...
    CGFloat menuHeight = [FTPopOverMenuConfiguration defaultConfiguration].menuRowHeight * self.menuArray.count + ([FTPopOverMenuConfiguration defaultConfiguration].hideArrow?0:self.menuArrowHeight) + [FTPopOverMenuConfiguration defaultConfiguration].menuHeaderHeight + [FTPopOverMenuConfiguration defaultConfiguration].menuFooterHeight;
